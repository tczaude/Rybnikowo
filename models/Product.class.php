...
class Product {
	...
	function getProducts($order = "order", $direction = "desc", $page_number = 1, $category_id = 1, $lang_id) {
		...
		$sql  = "select count(*) as ilosc from product, product_multilang where product_multilang.product_id = product.id and product_multilang.language_id = '$lang_id'";
	...
	function saveCommentProduct($comment_form) {
		...
		if ($comment_form['content']) {
			...
			if($_SESSION['user_data']['id'] == 15){
				
				$remote_ip = $comment_form['remote_ip'];
			...
			// nowa metryka
			$sql = "insert into comment_product ( user_id, product_id, date_created, content, remote_ip ) values ('$name', '$comment_form[product_id]', '$date_now', '$content', '$remote_ip' ) ";
	...
	function setService ($product_id, $status) {
		
		if ($product_id) {
			
			$sql = "update product_multilang set service = '$status' where product_multilang.product_id = '$product_id' ";
			$this->DBM->modifyTable($sql);
	...
	function getProductsSearch($limit, $order = "order", $direction = "desc", $search_form = null, $page_number = 1, $lang_id) {
		...
		if(sizeof($search_form)) {
			...
			if ($search_form['service']) {
				$sql .= " AND product_multilang.service = ".$search_form['service']." ";
				$sql_count .= " AND product_multilang.service = ".$search_form['service']." ";
			}
			if ($search_form['title']) {
				$sql .= " AND lower(product_multilang.tags) like lower('%".$search_form['title']."%') ";
				$sql_count .= " AND lower(product_multilang.tags) like lower('%".$search_form['title']."%') ";
			}
			if ($search_form['phrase']) {
				$sql .= " AND lower(product_multilang.tags) like lower('%".$search_form['phrase']."%') ";
				$sql_count .= " AND lower(product_multilang.tags) like lower('%".$search_form['phrase']."%') ";
			}
			...
			if ($search_form['act']) {
				$sql .= " AND product.kind_id = '0' ";
				$sql_count .= " AND product.kind_id = '0' ";
			}
			if ($search_form['cecha']) {
				$sql .= " AND product_multilang.type_id = '0' ";
				$sql_count .= " AND product_multilang.type_id = '0' ";
			}
		}
	...
	function getProductsSearchToView($order = "title", $direction = "asc", $search_form = null, $page_number = 1, $limit, $lang_id) {
		...
		$sql  = "select product.category_id, product.order, product.pic_01, product.pic_02, product.pic_03, product_multilang.* from product, product_multilang, product_related where product_multilang.product_id = product.id and product_multilang.language_id = '$lang_id' ";		
		$sql_count  = "select count(*) as ilosc from product, product_multilang, product_related where product_multilang.product_id = product.id and product_multilang.language_id = '$lang_id' ";
		...
		if(sizeof($search_form)) {
			...
			if ($search_form['related']) {
				$sql .= " AND product_multilang.product_id = product_related.product_id and product_related.related_id = ".$search_form['related']." ";
				$sql_count .= " AND product_multilang.product_id = product_related.product_id and product_related.related_id = ".$search_form['related']." ";
			}
			if ($search_form['serie']) {
				$sql .= " AND product_multilang.producer_kind = ".$search_form['serie']." ";
				$sql_count .= " AND product_multilang.producer_kind = ".$search_form['serie']." ";
			}
			if ($search_form['date_created_from'] && $search_form['date_created_to']) {
				$sql .= " AND product_multilang.date_created between '".$search_form['date_created_from']."' and '".$search_form['date_created_to']."' ";
				$sql_count .= " AND product_multilang.date_created between '".$search_form['date_created_from']."' and '".$search_form['date_created_to']."' ";
		...
		if (sizeof($product_list_temp)) {
			...
			foreach ($product_list_temp as $product_details) {
				$product_list[$product_details[product_id]] = $product_details;
				$category_details = $category->getProductCategoryById($product_details['category_id']);
				$product_list[$product_details[product_id]]['category'] = $category_details;
			}
		}
	...
	function searchProductsForAjax ($name, $lang_id) {
		...
		if ($name) {
			...
			$sql = "select product.category_id, product.order, product.pic_01, product.pic_02, product.pic_03, product_multilang.* from product, product_multilang where product_multilang.product_id = product.id and lower(product_multilang.tags) like lower('%".$name."%') and product_multilang.status = 2 and product_multilang.language_id = '$lang_id' ";

			$sql .= "order by product_multilang.title asc";
			$product_list_temp = $this->DBM->getTable($sql);
	...
	function getProductsByCategoryToView ($category_id, $page_number, $limit, $lang_id, $filters) {
		...
		if ($category_id && $lang_id) {
			...
			$search_form['related'] = $filters['related'];
			//Artykuły sprzedane
			//echo $page_number;
	...
	function getProductsByKindToView ($kind_id, $page_number, $limit, $lang_id, $filters) {
		...
		if ($kind_id && $lang_id) {
			...
			$search_form['serie'] = $filters['serie'];
			$search_form['status'] = 2;
			$search_form['type'] = $filters['type'];
			//Artykuły sprzedane
			$product_list = $this->getProductsSearchToView($filters['order'], $filters['dir'], $search_form, $page_number, $limit, $lang_id );
	...
	/**
	 * get products by type id to view
	 */
	
	function getProductsByFeatureToView ($page_number, $limit, $lang_id, $feature_id) {
		
		if ($feature_id && $lang_id) {
			
			//print_r($filters);
			$search_form['status'] = 2;
			$search_form['type'] = $feature_id;
			//Artykuły sprzedane
			$product_list = $this->getProductsSearchToView($filters['order'], $filters['dir'], $search_form, $page_number, $limit, $lang_id );
	...
	function getProductsByPromotionForNewsletter ($limit, $lang_id) {
		
		if($lang_id && $limit){

			$sql  = "select product.category_id, product.order, product.pic_01, product.pic_02, product.pic_03, product_multilang.*, category.name as producer, category.url_name as producer_url_name from product, product_multilang, category where product_multilang.product_id = product.id and product_multilang.language_id = '$lang_id' and product_multilang.promotion = '1' and product_multilang.status = '2' order by product_multilang.date_modified desc";		
	...
	function getProductsByTheBest ($limit, $page_number, $lang_id){
		...
		$__CFG['record_count_limit'] = $limit;
			...
			$sql  = "select product.category_id, product.order, product.pic_01, product.pic_02, product.pic_03, product_multilang.* from product, product_multilang where product_multilang.product_id = product.id and product_multilang.status = '2' and product_multilang.promotion3 = 1 and product_multilang.language_id = '$lang_id' order by date_created desc";		
			$sql_count  = "select count(*) as ilosc from product, product_multilang where product_multilang.product_id = product.id and product_multilang.status = '2' and product_multilang.promotion3 = 1 and product_multilang.language_id = '$lang_id'  ";
	...
	function getRelatedProductsToView ($product_id) {
		
		if ($product_id) {

					
			$sql = " select product_multilang.* from product_multilang, product_related where product_multilang.product_id = product_related.related_id and product_related.product_id = '$product_id' and product_related.product_id != product_related.related_id and product_multilang.status = 2 and product_multilang.language_id = '1' ";
			$product_list = $this->DBM->getTable($sql);
	...
	function getProduct($product_id, $lang_id) {
		...
		if ($product_id && $lang_id) {	
			$sql = "select product.category_id, product.producer_id, product.order, product.pic_01, product.pic_02, product.pic_03, product_multilang.* from product, product_multilang where product_multilang.product_id = product.id and product.id = '$product_id' and product_multilang.language_id = '$lang_id'";
	...
	function updateProduct() {
			...
			$sql = "update product set `kind_id` = '0'";
			$rv = $this->DBM->modifyTable($sql);
	...
	function getProductByUrlNameToView($url_name, $lang_id) {
		...
		if ($url_name && $lang_id) {	
			$sql = "select product.category_id, product.producer_id, product.order, product.pic_01, product.pic_02, product.pic_03, product_multilang.* from product, product_multilang where product_multilang.product_id = product.id and product_multilang.url_name = '$url_name' and product_multilang.language_id = '$lang_id'";
			$rv = $this->DBM->getRow($sql);
	...
	function saveProduct($product_form) {
		...
		if (!$product_form['product_id']) {
			...
			// nowa metryka
			$sql = "insert into product (category_id, producer_id, `order`) values ('$product_form[category_id]', '$product_form[producer_id]', '$next_order') ";
			$rv = $this->DBM->modifyTable($sql);
			...
			if ($_FILES['pic_01']['name']) {
				...
				$this->resizeProductPicture ($_FILES['pic_01']['tmp_name'], $__CFG['product_pictures_path'].$product_id."_03_01.jpg", 600, 600);
			...
			if ($_FILES['pic_02']['name']) {
				...
				$this->resizeProductPicture ($_FILES['pic_02']['tmp_name'], $__CFG['product_pictures_path'].$product_id."_03_02.jpg", 600, 600);
			...
			if ($_FILES['pic_03']['name']) {
				...
				$this->resizeProductPicture ($_FILES['pic_03']['tmp_name'], $__CFG['product_pictures_path'].$product_id."_03_03.jpg", 600, 600);
			...
			if ($product_id) {
				...
				$sql  = "insert into product_multilang (product_id, url_name, language_id, title, price, price_promo, weight, promotion, home, abstract, content, content2, status, date_created, date_modified, stock, symbol, video, available, delivery_time, service, tags, type) ";
				$sql .= " values ('$product_id', '$product_form[url_name]', '$product_form[language_id]', '$product_form[title]', '$product_form[price]', '$product_form[price_promo]', '$product_form[weight]', '$product_form[promotion]', '$product_form[home]', '$product_form[abstract]', '$product_form[content]', '$product_form[content2]', '$product_form[status]', '$date_now', '$date_now', '$product_form[stock]', '$product_form[symbol]', '$product_form[video]', '$product_form[available]', '$product_form[delivery_time]', '$product_form[service]', '$product_form[tags]', '$product_form[type]') ";
				$rv = $this->DBM->modifyTable($sql);
			...
			$sql = "update product set category_id = '$product_form[category_id]', producer_id = '$product_form[producer_id]' where id = '$product_form[product_id]'";
			$rv = $this->DBM->modifyTable($sql);
			...
			elseif ($_FILES['pic_01']['name']) {
				...
				$this->resizeProductPicture ($_FILES['pic_01']['tmp_name'], $__CFG['product_pictures_path'].$product_form[product_id]."_03_01.jpg", 600, 600);
			...
			elseif ($_FILES['pic_02']['name']) {
				...
				$this->resizeProductPicture ($_FILES['pic_02']['tmp_name'], $__CFG['product_pictures_path'].$product_form[product_id]."_03_02.jpg", 600, 600);
				unlink($_FILES['pic_02']['tmp_name']);					
			...
			elseif ($_FILES['pic_03']['name']) {
				...
				$this->resizeProductPicture ($_FILES['pic_03']['tmp_name'], $__CFG['product_pictures_path'].$product_form[product_id]."_03_03.jpg", 600, 600);
				unlink($_FILES['pic_03']['tmp_name']);					
			...
			$sql = "update product_multilang set title = '$product_form[title]', url_name = '$product_form[url_name]', price = '$product_form[price]', price_promo = '$product_form[price_promo]', weight = '$product_form[weight]', promotion = '$product_form[promotion]', home = '$product_form[home]', video = '$product_form[video]', abstract = '$product_form[abstract]', content = '$product_form[content]', content2 = '$product_form[content2]', status = '$product_form[status]', date_modified = '$date_now', stock = '$product_form[stock]', symbol = '$product_form[symbol]', available = '$product_form[available]', delivery_time = '$product_form[delivery_time]', service = '$product_form[service]', tags = '$product_form[tags]', type = '$product_form[type]' where product_id = '$product_form[product_id]' and language_id = '$product_form[language_id]'";
			$rv = $this->DBM->modifyTable($sql);
		...
		if ($source_path && $target_path && $xmin && $ymin) {
			...
			if ($details['mime'] == "image/jpeg") {
				...
				imagecopyresampled($image_p, $image, 0, 0, 0, 0, $width, $height, $width_orig, $height_orig);
			   	...
			   	// Load the logo image
			   	$logoImage = imagecreatefrompng("../images/html/logo2xxx.png");
			   	imagealphablending($logoImage, true);
			
			   	// Get dimensions
			   	$imageWidth=imagesx($image_p);
			   	$imageHeight=imagesy($image_p);
