...
<tr>
	<td width="100%" valign="top" align="center" colspan="1">
		...
		<tr>
			...
			{include file="filters_auction.tpl"}
			{include file="paging_auction.tpl"}
				...
				<tr>
					<td>
						...
						{foreach from=$auctions_list item=auction name=auction}
						
						{cycle name=color assign=row_color values="#FFFFFF,#EEEEE1"}
						
						<tr bgcolor="{$row_color}">
							
							<td width="50%" align="left" {if $auction.status ne 1}style="color:#bbbbbb;"{/if}>
								<span style="font-weight:italic;font-size:9px;color:#bbbbbb;">Dnia : {$auction.date_created}</span><br>
								{$auction.surname}&nbsp;{$auction.name}
							</td>
							<td width="10%" style="text-align: left;">{$auction.city}</td>
							<td width="10%" style="text-align: right;">{$auction.amount}&nbsp;PLN</td>
							<td width="10%">&nbsp;</td>
							<td nowrap align="center">
								<a href="javascript: openwin('{$path}/auction/display/{$auction.id}/{$language.id}');"><img src="{$path}/images/magnifier.png" border="0" title="{$dict_templates.Edit} ({$language.short})"></a>&nbsp;
							</td>
							
							<td>
								<a href="remove.php" onclick="if (confirm('Czy na pewno chcesz usunąć wybraną kategorię?')) window.location='{$path}/auction/remove/{$auction.id}'; return false;"><img src="{$path}/images/cross.png" border="0" title="{$dict_templates.Remove}"></a>
							</td>
							{*
							<td>
